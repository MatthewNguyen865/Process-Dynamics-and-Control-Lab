...
from utils.data_export import save_trajectory_to_csv
...
save_trajectory_to_csv(times, temperatures, filename=f"{BASIN_DATA_DIR}/uncontrolled_basin_dynamics.csv")
...
save_trajectory_to_csv(times, temperatures, filename=f"{BASIN_DATA_DIR}/pid_controlled_basin_dynamics.csv")
...
for trajectory, label in zip(trajectories, labels):
    ...
    save_trajectory_to_csv(times, trajectory, filename=f"{DISTURBANCE_DATA_DIR}/{filename}.csv")
